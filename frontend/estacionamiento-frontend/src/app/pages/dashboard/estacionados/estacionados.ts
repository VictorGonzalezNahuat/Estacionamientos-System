...
export class Estacionados implements OnInit, OnDestroy {
  ...
  private mapEstacionado(item: any): EstacionadoRow {
    ...
    return {
      ...
      fechaEntrada: entryDate ? this.formatDate(entryDate) : this.displayValue(item?.fecha_entrada),
      horaEntrada: entryDate ? this.formatTime(entryDate) : this.displayValue(item?.hora_entrada),
